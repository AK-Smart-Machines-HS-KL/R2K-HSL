...
## 2026-08-25 (cont.4) — uni-mainz Qwen3 235B context fixed: instructions trimmed + limit declared

**Goal:** Qwen3 235B Thinking on uni-mainz errors with 49,221 tokens on "hi" (model has ~32k context). Make opencode less greedy.

**Done:**
- **Diagnosis:** The 49,221 tokens came from workspace instructions — `SESSION_CHANGELOG.md` alone was 166KB (~40k tokens, 77% of payload). Total injected: 216KB ≈ 54k tokens. uni-mainz 235B has 32k context → always broken regardless of what message you type.
- **Workspace fix:** Removed `SESSION_CHANGELOG.md` from `.opencode/opencode.json` instructions. Instructions now: router (28KB) + agent_prompt (6.3KB) = ~14k tokens. Every model's prefill drops 49k → 14k (3.5× faster local, cheaper paid APIs, uni-mainz now usable). Session continuity preserved via AGENTS.md's "READ THIS FIRST after reboot" — agent reads the changelog tail on demand.
- **Model limit declared:** Added `"limit": {"context": 32768, "output": 8192}` to `Qwen3 235B Thinking` in `~/.config/opencode/opencode.json`. opencode now knows the ceiling and auto-compacts conversation correctly. (Config schema supports `limit.context` per custom model.)
- **v6.7 final benchmark IS RUNNING** (detached setsid, 16:14, ~17/100 done, ETA ~22:30). Independent of opencode, survives restart. Avoid local 32B usage until it completes to prevent polluting latency KPIs.

**Files touched:**
- `.opencode/opencode.json` — removed SESSION_CHANGELOG.md from instructions
- `~/.config/opencode/opencode.json` — added `limit.context/output` to Qwen3 235B
- `docs/SESSION_CHANGELOG.md` — this entry

**Files deleted:** None

**Not yet done:**
- Restart opencode to activate new config + verify uni-mainz 235B with "hi"
- After benchmark (~22:30): analyze results, re-baseline kpi_targets, commit
- v7 TeamCaptain design doc

**Next:**
1. Restart opencode, verify uni-mainz Qwen3 235B works
2. After benchmark completes: analyze results, re-baseline kpi_targets.json, commit
3. v7 TeamCaptain design

**Blockers:** None. GPU healthy. Configs edited. Benchmark running in background.

---
<!-- Stub generated by session_entry.sh on 2026-08-25 -->

## 2026-08-25 (cont.5) — Changelog growth prevention + archival plan designed

**Goal:** Design mechanisms to prevent unbounded changelog growth; create
execution-ready plan for tomorrow's archival session.

**Done:**
- **4 growth-prevention mechanisms designed:**
  - M1 (tail-read protocol): AGENTS.md line 13 changed from "READ THIS FIRST"
    to "READ THE LAST ~150 LINES FIRST" — caps session-start token cost at
    ~1.6k regardless of file size. Decouples file growth from token cost.
  - M2 (size guard): `session_entry.sh` gets a 100 KB size guard — prints a
    prominent warning when the active changelog exceeds 100 KB, pointing to
    `docs/CHANGELOG_ARCHIVAL.md` for the archival procedure.
  - M3 (entry discipline): keep entries ≤~40 lines; findings go to power files
    or `src/results/*.md`, the entry links them. User decision: "rely on
    discipline" (no hard cap enforced by tooling).
  - M4 (institutionalized protocol): `docs/CHANGELOG_ARCHIVAL.md` written
    with 5-phase procedure (distillation sweep → boundary measurement →
    entry-granular move → reorder → verification → session entry + commit).
- **Sequencing decision:** bundle M1-M4 edits into tomorrow's archival session
  (one coherent docs change).
- **Threshold decision:** 100 KB guard (aligns with ~monthly archival cadence
  post-discipline).
- **Cutoff decision:** B (2026-08-23, ~45 KB / 11 entries kept).
- **Commit decision:** direct `docs:` commit on main, no branch.
- **Recon measurements captured** in `docs/CHANGELOG_ARCHIVAL.md` Appendix:
  entry boundaries, byte sizes per cutoff, out-of-order entries, distillation
  debt status.
- **Distillation debt verified:** 2026-08-24 AKM/forensics numbers PAID in
  8_C3_SOCCER_KNOWLEDGE.md:750-767. jsonq numbers (55.7%/50.7% closest,
  57.8%/64.4% over-kick) NOT PAID — Phase 1 of archival session must add
  these to the KB before archiving the 2026-08-25 (cont.2) entry.

**Files touched:**
- `docs/CHANGELOG_ARCHIVAL.md` (NEW — execution-ready archival protocol)
- `docs/SESSION_CHANGELOG.md` — this entry

**Files deleted:** None

**Not yet done:**
- Restart opencode to activate uni-mainz config fix (cont.4's Step 3)
- v6.7 benchmark completing (~22:30 tonight) → analyze → re-baseline → commit
- CHANGELOG_ARCHIVAL.md session (tomorrow, after v6.7 commit):
  1. M1 edit: AGENTS.md line 13 (tail-read protocol)
  2. M2 edit: session_entry.sh size guard (100 KB)
  3. Phase 1: add jsonq numbers to 8_C3_SOCCER_KNOWLEDGE.md (distillation debt)
  4. Phases 2-4: archive entries pre-2026-08-23 per the protocol
  5. Phase 5: session entry + commit
- v7 TeamCaptain design doc

**Next:**
1. Restart opencode, verify uni-mainz Qwen3 235B with "hi" (15k < 32k now)
2. After benchmark (~22:30): `python3 tools/analyze_trace.py --run-id <latest>`
   → re-baseline kpi_targets → commit v6.7
3. Next session: execute CHANGELOG_ARCHIVAL.md protocol

**Blockers:** None. GPU healthy. Configs edited. Benchmark running (ETA ~22:30).
Archival plan ready in `docs/CHANGELOG_ARCHIVAL.md:1-228`.

## 2026-08-25 (cont.6) — Edge-LLM project proposal drafted

**Goal:** Minimal entry per session protocol. Edge-LLM/on-board model exploration
deferred to post-v6.7.

**Done:**
- `docs/proposal_edge_llm_k1.md` — team proposal for on-board LLM on AGX Orin
  for K1. German, with technical footnotes and source references.
