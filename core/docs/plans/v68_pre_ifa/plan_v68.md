...
| V5  | **Odom watch for calib_cli** — `--odom` mode: subscribe ~~`/Kev1n/odometer_state`~~ **[FOLKLORE — silent placeholder, audit §5 k1_kick_head_vendor_audit.md; K1 side probe-gated on `rt/odom` ROS bridge]** + Yahboom `/blue_N/odom_raw` (**verified live 2026-09-04** — physical odom, NOT `/blue_N/odom` which is the sim twin), CSV-log commanded-vs-reported per task                                                                                                                                    | `core/tools/calib_cli.py`                                                                         | lab session produces first drift CSV                               | 0.5 d      |
