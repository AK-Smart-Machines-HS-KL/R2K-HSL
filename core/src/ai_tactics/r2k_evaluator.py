...
# --- Demo mode: sequence-based waypoint tracking ---
...
# tracks the current target label, detects arrival, and injects the label
# into the user prompt. The LLM does pure string→coordinate lookup.
_demo_waypoints = []       # [{"label": "FIRST", "x": -1.0, "y": 1.5, "hold_duration": 0}, ...]
_demo_target_idx = 0       # current index into _demo_waypoints
_demo_stopped_idx = 0      # saved target_idx when "stop" was pressed (for "resume")
_demo_arrival_dist = 0.5   # bot within this distance → advance to next WP
_demo_arrival_time = 0     # timestamp when bot arrived at current WP (for hold_duration)
...
# START position — bot starts at center
_demo_start_pos = (0.0, 0.0)

def _load_demo_waypoints():
    """Load waypoints from shared_state/waypoints.json."""
    global _demo_waypoints
    try:
        with open(WAYPOINTS_PATH, 'r') as f:
            data = json.load(f)
        wps = data.get("waypoints", [])
        _demo_waypoints = [{"label": w["label"], "x": float(w["x"]), "y": float(w["y"]),
                            "hold_duration": float(w.get("hold_duration", 0))} for w in wps]
    except Exception:
        _demo_waypoints = []

def _demo_target_for_bot(bx, by):
    """Return the current target label and advance if the bot has arrived.
    Does NOT loop — when the last waypoint is reached, the bot stays there."""
    global _demo_target_idx, _demo_arrival_time
    if not _demo_waypoints:
        return None
    wp = _demo_waypoints[_demo_target_idx]
    dist = math.hypot(bx - wp["x"], by - wp["y"])
    if dist < _demo_arrival_dist:
        hold = wp.get("hold_duration", 0)
        if hold > 0:
            if _demo_arrival_time == 0:
                _demo_arrival_time = time.time()
                return wp["label"]
            elif time.time() - _demo_arrival_time < hold:
                return wp["label"]
        # Advance to next WP, but don't loop past the last one
        if _demo_target_idx + 1 < len(_demo_waypoints):
            _demo_target_idx = _demo_target_idx + 1
            _demo_arrival_time = 0
            wp = _demo_waypoints[_demo_target_idx]
        # else: stay at last waypoint (no loop)
    return wp["label"]

def _compile_demo_task(task_text, ents, match_state):
    """One-shot LLM call: translate NL task to waypoint list. Returns True on success."""
    global _demo_target_idx, _demo_arrival_time
    ...
    world_lines.append(f"  START position: ({_demo_start_pos[0]}, {_demo_start_pos[1]})")

    landmarks_text = "\n".join(f"  {name.upper()} at ({x}, {y})" for name, (x, y) in DEMO_LANDMARKS.items())

    sys_prompt = f"""You are a robot path planner. Convert the task into a list of named waypoints.
...
"""
    ...
    try:
        ...
        if isinstance(wps, list) and len(wps) > 0:
            waypoints = [{"label": w["label"], "x": float(w["x"]), "y": float(w["y"]),
                          "hold_duration": float(w.get("hold_duration", 0))} for w in wps]
...
def _write_move_strategy(x, y):
    """Write a Move command directly to current_strategy.json (bypasses LLM)."""
    try:
        strategy = {"assignments": {"blue_1": {"action": "Move", "x": x, "y": y}}}
        with open(STRATEGY_PATH + ".tmp", 'w') as f:
            json.dump(strategy, f)
        os.replace(STRATEGY_PATH + ".tmp", STRATEGY_PATH)
    except Exception:
...
def _check_task_input(ents, match_state):
    ...
    global _last_task_mtime, _demo_waypoints, _demo_target_idx, _demo_arrival_time, _demo_start_pos
    ...
    try:
        ...
        with open(TASK_INPUT_PATH, 'r') as f:
            data = json.load(f)
        task_text = data.get("task", "").strip().lower()
        if not task_text:
            return
        ...
        # Saves the current target index so "resume" can continue from here
        if task_text in ("stop", "break", "exit", "halt"):
            global _demo_stopped_idx
            _demo_stopped_idx = _demo_target_idx
            _demo_waypoints = []
            _demo_arrival_time = 0
            _write_hold_strategy()
            print(f"⚡ STOP: bot halted at current position (was heading to waypoint {_demo_stopped_idx})", flush=True)
            return

        # RESUME / CONTINUE: recover from "stop" and follow the remaining waypath
        # Reloads waypoints from file and continues from where the bot stopped
        if task_text in ("resume", "continue"):
            if not _demo_waypoints:
                _load_demo_waypoints()
            if _demo_waypoints:
                _demo_target_idx = _demo_stopped_idx if _demo_stopped_idx < len(_demo_waypoints) else 0
                _demo_arrival_time = 0
                labels = [w["label"] for w in _demo_waypoints]
                print(f"▶ RESUME: continuing from waypoint {_demo_target_idx} ({labels[_demo_target_idx] if _demo_target_idx < len(labels) else '?'})", flush=True)
            else:
                print(f"▶ RESUME: no waypoints to resume", flush=True)
            return

        # RESTART / REDO / REPEAT: replay the last waypath from the beginning
        if task_text in ("restart", "re-start", "redo", "repeat"):
            if not _demo_waypoints:
                _load_demo_waypoints()
            if _demo_waypoints:
                _demo_target_idx = 0
                _demo_arrival_time = 0
                labels = [w["label"] for w in _demo_waypoints]
                print(f"▶ RESTART: replaying {len(_demo_waypoints)} waypoints from start: {labels}", flush=True)
            else:
                print(f"▶ RESTART: no waypoints to restart", flush=True)
            return

        # GO HOME / RETURN: stop waypath, drive to START position
        if task_text in ("go home", "return", "return to start", "go to start", "home"):
            _demo_waypoints = []
            _demo_target_idx = 0
            _demo_arrival_time = 0
            sx, sy = _demo_start_pos
            _write_move_strategy(sx, sy)
            print(f"🏠 GO HOME: bot driving to START at ({sx}, {sy})", flush=True)
            return

        # --- Normal path: compile task via 7B LLM ---
        _compile_demo_task(task_text, ents, match_state)
...
def main():
    ...
    # Demo mode: load waypoints from shared_state/waypoints.json
    _load_demo_waypoints()
    if _demo_waypoints:
        print(f"Demo mode: {len(_demo_waypoints)} waypoints loaded from {WAYPOINTS_PATH}", flush=True)
    ...
    while True:
        try:
            ...
            if _active_mode == "demo":
                ...
                # If stopped (no waypoints), skip LLM call — the fast-path
            ...
            if TEXT_MODE:
                ...
                # Demo mode: inject current target label + waypoint table for the bot
                if _active_mode == "demo":
                    for bname in sorted(k for k in ents if k.startswith("blue")):
                        bv = ents[bname]
                        target_label = _demo_target_for_bot(bv["x"], bv["y"])
                        if target_label:
                            min_ents["target"] = target_label
                    # Inject current waypoint table so the LLM has the correct coordinates
                    # (waypoints.json may have been recompiled interactively — the system
                    # prompt's rules_demo.txt has stale coords)
                    if _demo_waypoints:
                        wp_table = {wp["label"]: {"x": wp["x"], "y": wp["y"]} for wp in _demo_waypoints}
